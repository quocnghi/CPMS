app.controller("EditCTDTKHStep4Controller", function ($scope, EditCTDTKHStep4Service, $window, $location) {
    getallInfo();
    $scope.tenGV = [];
    $scope.emails = [];
    $scope.giangVien = [];
    ...
    function getallInfo() {
        EditCTDTKHStep4Service.getInfo().success(function (pro) {
    ...
    $scope.addGV = function (mamh, dcuong, index) {
        ...
        console.log(data);
        if (dcuong != undefined) {
            EditCTDTKHStep4Service.editVien(data).success(function (msg) {
                console.log(msg);
    ...
    $scope.saveEmail = function (e) {
        ...
        EditCTDTKHStep4Service.saveEmail(e).success(function (ret) {
    ...
    $scope.finish = function (e) {
        EditCTDTKHStep4Service.sendEmail(e.MaET).success(function (ret) {
            toastr.success(ret.msg);
            $window.location.href = '/CTDTKehoach/Index';
        }, function () {
            alert('Có lỗi xảy ra');
        });
    }
    ...
    $scope.chooseEmail = function (date) {
        ...
        if (count == slmonhoc) {
            ...
            model.modal('show');
            EditCTDTKhungStep4Service.chEmail().success(function (ret) {
                console.log(ret)
