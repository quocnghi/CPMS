...
function changeNganh() {
    ...
    $.ajax({
        ...
        type: 'POST',
        url: '/CMS/CTDTKeHoach/changeNganh',
        dataType: "json",
        contentType: "application/json",
        cache: false,
        error: function (data) {
        ...
        success: function (data) {
            $("select[name='nganh'] option").remove()
            //console.log(data.nganh[0])
            $.each(data.nganh, function (i, item) {           
                $("#nganh").append('<option value="' + item.MaHN + '">' + item.Mota + '</option>');
            });
        }
    });
}
...
document.getElementById('btnApply').addEventListener("click", function (event) {
    $('#the').html('');
    $('#noidung').html('<img style="position:inherit; float:left; padding-left:550px" src="/Areas/CMS/Content/Curriculum/5.gif" />');
    $('#monhoc').html('');
    $('#lop').html('');
    ...
    $.ajax({
        ...
        success: function (json) {
            ...
            if (json.Message == undefined) {
                ...
                var lop = ' <div class="control-group"><select class="select2 form-control custom-select" id = "khoilop" required><option value="" hidden>Chọn khối lớp</option>';
                $.each(json.khoilop, function (i, item) {
                    lop += '<option value="' + item.MaKhoi + '">' + item.TenKhoi + '</option>';
                });
                lop += '</select><div class="invalid-feedback">Chưa chọn khối lớp</div></div>';
                ...
                toastr.warning(json.Message, 'Thông báo');
                $('#noidung').html('');
            }
...
document.getElementById('btnAddSubject').addEventListener("click", function (event) {
    ...
    console.log(data);
    if (nganh == "") {
        $("#khoilop").addClass("is-invalid");
    } else {
        $("#khoilop").removeClass("is-invalid");
    }
    if (nganh != "") {
        $.ajax({
            async: true,
            ...
            success: function (json) {
                if (json.Message == undefined) {
                    ...
                    $.each(json.data, function (i, item) {
                        i++;
                        $('#zero_config tr:last').after('<tr><td style="text-align:center">' + i + '</td><td style="text-align:center">' + item.Trinhdo + '</td><td style="text-align:center">' + item.Mota + '</td><td style="text-align:center">' + item.TenKhoi + '</td><td style="text-align:center">' + item.Tinhtrang + '<td style="text-align:center"><a href="/CTDTKeHoach/EditCTDTKehoachStep1/' + item.MaCTDT + '"><img src="/Areas/CMS/images/IconEdit.png" style="width:28px;"></a><a title="Ðánh giá" href="#" data-toggle="modal" data-target="#evaluateCTDT" style="margin:5px;"><img src="/Areas/CMS/images/IconCheck2.png" style="width:35px;"></a></td></tr>');
                    });
                }
